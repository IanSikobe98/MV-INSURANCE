...
import { ProfileupService } from '../services/profileup/profileup.service';
import { ActivatedRoute, Router } from '@angular/router';
import { PopprofPageModule } from '../popprof/popprof.module';
import { PopprofPage } from '../popprof/popprof.page';
import { LoadingService } from '../services/loading/loading.service';
import { WaitService } from '../services/wait/wait.service';

@Component({
  selector: 'app-profileupd',
  templateUrl: './profileupd.page.html',
  styleUrls: ['./profileupd.page.scss'],
})
export class ProfileupdPage implements OnInit {
  ...
  constructor(private WaitService : WaitService,private  LoadingService : LoadingService ,private route: ActivatedRoute,private router: Router,private ProfileupService : ProfileupService,public LoadingController:LoadingController,private popover:PopoverController,public storage:Storage,public navCtrl: NavController, private SignupService:SignupService) { 
    this.route.queryParams.subscribe(params =>{
      ...
      console.log(this.data);
      this.poa=this.data[0].poa;
      // console.log(this.data[0].poa);

      this.pno=this.data[0].phonenumber;
      this.fna=this.data[0].fname;
      this.mna=this.data[0].mname;
      this.sna=this.data[0].sname;
      this.nid=this.data[0].natid;
      this.gen=this.data[0].gender;
      this.pa=this.data[0].pha;
     
      this.ga=this.data[0].ga; 
      this.occ=this.data[0].occupation;  
      this.town=this.data[0].town;  
      this.dob=this.data[0].dob; 
      // this.pass=this.data[0].poa; 
    })
    // console.log(this.data.poa);
...
if(this.valnew.valfull==0){
  ...
  this.LoadingService.present("Updating Details....");
  const wait = await this.WaitService.waitline();
    ...
    this.ProfileupService.update(this.user.pno,this.user.fna ,this.user.mna ,this.user.sna ,this.user.nid ,this.user.gen,this.user.pa 
      ,this.user.poa,this.user.ga ,this.user.occ ,this.user.town,this.user.dob,this.data[0].natid).subscribe((status:any) => {
      console.log(status);
      ...
      if(status){
              if(status.status == true){
                // this.storage.set('userdetails', JSON.stringify(status));  
                  // this.navCtrl.pop;
                  this.popover.create({component:PopprofPage,showBackdrop:false}).then((popoverElement)=>{
...
//     }
    ...
    validationfull(f1,f2,f3,f4,f5,f6,f7,f8,f9,f10,f11,f12,f13){
      ...
      if(this.valarr.valf1==1 || this.valarr.valf2==1 || this.valarr.valf3==1 || this.valarr.valf4==1 || this.valarr.valf6==1 || this.valarr.valf7==1|| this.valarr.valf8==1 || this.valarr.valf9==1 || this.valarr.valf10==1 || this.valarr.valf11==1 || this.valarr.valf12==1 )
      {
        this.valarr.valfull=1;
       
      }
      else if(this.valarr.valf1==0 && this.valarr.valf2==0 && this.valarr.valf3==0 && this.valarr.valf4==0  && this.valarr.valf6==0 && this.valarr.valf7==0 && this.valarr.valf8==0 && this.valarr.valf9==0 && this.valarr.valf10==0  && this.valarr.valf11==0&& this.valarr.valf12==0 ){
